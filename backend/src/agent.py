...
def get_mistral_client() -> Mistral:
    ...
    if not api_key:
        raise ValueError(
            "🚨 AI provider API key not set. Add MISTRAL_API_KEY to backend/.env."
        )
...
def analyze_risk_and_hedge(event_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Call the scenario analysis agent for a given space event.
...
{"reasoning": "<1-2 sentence explanation>", "action": "HEDGE" or "IGNORE", "hedge_amount_usd": <number>, "hedge_type": "collision_insurance" or "maneuver_fuel_cost" or "delay_insurance" or "none"}"""
    ...
    print(f"🧠 Asking the scenario engine to evaluate risk for: {display_name}")
    ...
    try:
        ...
        # Normalize to expected keys from the current AI provider.
        normalized = {
        ...
        print(f"❌ AI provider returned invalid JSON: {e}")
        return {
        ...
        err_msg = str(e)
        print(f"❌ AI provider error: {err_msg}")
        if "401" in err_msg or "Unauthorized" in err_msg:
            print("   → Check backend/.env: MISTRAL_API_KEY must be valid.")
            print("   → Ensure no extra spaces; enable billing if required.")
        return {
            "reasoning": f"Failed to reach AI provider: {err_msg}",
            "action": "IGNORE",
            "hedge_amount_usd": 0,
            "hedge_type": "none",
        }
